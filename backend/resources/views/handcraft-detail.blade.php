...
<section class="relative w-full py-12 md:py-20 bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900">
    <div class="max-w-6xl mx-auto px-6">
        <div class="mb-8">
            <a href="{{ route('izrabotki') }}" class="text-slate-400 hover:text-white text-sm font-medium inline-flex items-center gap-2 transition">
        ...
        <div class="mt-6 text-sm text-slate-400">
            Објавено: <time datetime="{{ $handcraft->published_at?->format('Y-m-d') }}">
                {{ $handcraft->published_at?->format('d.m.Y') ?? 'N/A' }}
            </time>
        </div>
    </div>
</section>
        ...
        <div class="hidden md:grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach($handcraft->images as $image)
                <div class="group relative overflow-hidden rounded-xl shadow-md hover:shadow-xl transition-shadow duration-300 h-72">
                    <img 
                        src="{{ $resolveUrl($image->image_url) }}"
                        alt="Слика {{ $loop->iteration }}"
                        class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300"
                    />
                    <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                        <button 
                            type="button"
                            onclick="openImageModal('{{ $resolveUrl($image->image_url) }}')"
...
<section class="py-12 md:py-16">
    <div class="max-w-6xl mx-auto px-6">
        ...
        <div class="grid md:grid-cols-3 gap-6">
            @foreach($relatedHandcrafts as $craft)
                <a 
                    href="{{ route('handcraft.detail', $craft) }}"
