...
<div class="db-alert-banner">
    ...
    <div class="db-alert-chips">
        @if(($canSeeContactAlerts ?? false) && ($newComplaintsCount ?? 0) > 0)
            <a href="{{ $isReviewer ? route('admin.complaints') : route('admin.reviewer-dashboard') }}" class="db-chip db-chip--red">
                <span>Нови жалби</span>
                <span class="db-chip-badge">{{ $newComplaintsCount }}</span>
            </a>
        @endif
        @if(($canSeeContactAlerts ?? false) && ($newComplimentsCount ?? 0) > 0)
            <a href="{{ route('admin.compliments') }}" class="db-chip db-chip--green">
                <span>Нови пофалби</span>
                <span class="db-chip-badge">{{ $newComplimentsCount }}</span>
            </a>
        @endif
        @if($isAdmin && ($visitRequestsSubmittedToday ?? 0) > 0)
            <a href="{{ route('admin.visit-requests') }}" class="db-chip db-chip--sky">
                <span>Нови барања денес</span>
                <span class="db-chip-badge">{{ $visitRequestsSubmittedToday }}</span>
            </a>
        @endif
        @if($isAdmin && ($visitsScheduledToday ?? 0) > 0)
            <a href="{{ route('admin.visit-requests') }}" class="db-chip db-chip--blue">
                <span>Посети за денес</span>
                <span class="db-chip-badge">{{ $visitsScheduledToday }}</span>
            </a>
        @endif
    </div>

    @if($isAdmin && ($urgentVisitRequests ?? collect())->isNotEmpty())
        <div class="db-alert-list-section">
            <p class="db-alert-list-label">Барања / посети за денес</p>
            <ul class="db-alert-list">
                @foreach($urgentVisitRequests as $visit)
                    <li class="db-alert-list-item">
                        <i class="fas fa-calendar-day db-alert-list-icon"></i>
                        <span>
                            <strong>{{ trim($visit->visitor_first_name.' '.$visit->visitor_last_name) }}</strong>
                            — термин {{ $visit->visit_date?->format('d.m.Y') ?? '—' }}
                            <span class="db-alert-list-meta">({{ $visitStatusLabels[$visit->status] ?? $visit->status }})</span>
                        </span>
                    </li>
                @endforeach
            </ul>
    ...
    @if(($canSeeContactAlerts ?? false) && ($urgentNewComplaints ?? collect())->isNotEmpty())
        <div class="db-alert-list-section">
            <p class="db-alert-list-label">Нови жалби</p>
            <ul class="db-alert-list">
                @foreach($urgentNewComplaints as $complaint)
                    <li class="db-alert-list-item">
                        <i class="fas fa-exclamation-circle db-alert-list-icon db-alert-list-icon--red"></i>
                        <span>
                            <strong>{{ $complaint->submitted_by_name }}</strong>
                            — {{ \Illuminate\Support\Str::limit($complaint->subject, 60) }}
                        </span>
                    </li>
                @endforeach
            </ul>
...
<div class="db-grid-4 mb-section">
    ...
    <div class="db-stat-card">
        ...
        <div class="db-stat-body">
            ...
            <span class="db-stat-value">{{ $activitiesCount ?? 0 }}</span>
    ...
    <div class="db-stat-card">
        ...
        <div class="db-stat-body">
            ...
            <span class="db-stat-value">{{ $announcementsCount ?? 0 }}</span>
    ...
    <div class="db-stat-card">
        ...
        <div class="db-stat-body">
            ...
            <span class="db-stat-value">{{ $galleryCount ?? 0 }}</span>
    ...
    <div class="db-stat-card">
        ...
        <div class="db-stat-body">
            ...
            <span class="db-stat-value">{{ $handcraftsCount ?? 0 }}</span>
...
{{-- ═══════════════════════════════════════════════════════════
     ROW 2 — VISIT & CONTACT STATS
═══════════════════════════════════════════════════════════ --}}
<p class="db-section-label">Посети и контакт</p>
<div class="db-grid-4 mb-section">

    <div class="db-stat-card {{ $isAdmin && ($visitRequestsSubmittedToday ?? 0) > 0 ? 'db-stat-card--highlight-sky' : '' }}">
        <div class="db-stat-icon db-stat-icon--sky">
            <i class="fas fa-calendar-check"></i>
            @if($isAdmin && ($visitRequestsSubmittedToday ?? 0) > 0)
                <span class="db-stat-badge">{{ $visitRequestsSubmittedToday }}</span>
            @endif
        </div>
        <div class="db-stat-body">
            <span class="db-stat-label">Барања за посета</span>
            <span class="db-stat-value">{{ $visitRequestsCount ?? 0 }}</span>
            @if($isAdmin && ($visitRequestsSubmittedToday ?? 0) > 0)
                <span class="db-stat-hint db-stat-hint--sky">{{ $visitRequestsSubmittedToday }} нови денес · вкупно</span>
            @else
    ...
    <div class="db-stat-card">
        ...
        <div class="db-stat-body">
            ...
            <span class="db-stat-value">{{ $approvedRequestsCount ?? 0 }}</span>
            <span class="db-stat-hint">Посети со статус Одобрено</span>
        </div>
    </div>

    <div class="db-stat-card {{ ($canSeeContactAlerts ?? false) && ($newComplaintsCount ?? 0) > 0 ? 'db-stat-card--highlight-red' : '' }}">
        <div class="db-stat-icon db-stat-icon--red">
            <i class="fas fa-comments"></i>
            @if(($canSeeContactAlerts ?? false) && ($newComplaintsCount ?? 0) > 0)
                <span class="db-stat-badge db-stat-badge--red">{{ $newComplaintsCount }}</span>
            @endif
        </div>
        <div class="db-stat-body">
            <span class="db-stat-label">Жалби</span>
            <span class="db-stat-value">{{ $complaintsCount ?? 0 }}</span>
            @if(($canSeeContactAlerts ?? false) && ($newComplaintsCount ?? 0) > 0)
                <span class="db-stat-hint db-stat-hint--red">{{ $newComplaintsCount }} нови · вкупно</span>
            @else
    ...
    <div class="db-stat-card">
        ...
        <div class="db-stat-body">
            ...
            <span class="db-stat-value">{{ $complimentsCount ?? 0 }}</span>
...
<div class="db-grid-3col mb-section">
    ...
    <div class="db-card">
        ...
        <div class="db-actions-grid">
            <a href="{{ route('admin.main-activities.create') }}" class="db-action-btn db-action-btn--primary">
                <i class="fas fa-plus-circle"></i>
                <span>Нова активност</span>
            </a>
            <a href="{{ route('admin.announcements.create') }}" class="db-action-btn db-action-btn--primary">
                <i class="fas fa-file-circle-plus"></i>
                <span>Ново соопштение</span>
            </a>
            <a href="{{ route('admin.gallery.create') }}" class="db-action-btn db-action-btn--primary">
                <i class="fas fa-image"></i>
                <span>Нова слика</span>
            </a>
            <a href="{{ route('admin.izrabotki') }}" class="db-action-btn db-action-btn--primary">
                <i class="fas fa-hammer"></i>
                <span>Уреди изработки</span>
            </a>
            <a href="{{ route('admin.aboutus') }}" class="db-action-btn db-action-btn--secondary">
                <i class="fas fa-info-circle"></i>
                <span>Уреди „За Нас"</span>
            </a>
            <a href="{{ route('admin.visit-schedules') }}" class="db-action-btn db-action-btn--secondary">
                <i class="fas fa-clock"></i>
                <span>Распоред на посети</span>
            </a>
            @if ($isAdmin)
                <a href="{{ route('admin.visit-requests') }}" class="db-action-btn db-action-btn--secondary">
                    <i class="fas fa-calendar-check"></i>
                    <span>Барања за посета</span>
                </a>
                <a href="{{ route('admin.reviewer-dashboard') }}" class="db-action-btn db-action-btn--secondary">
                    <i class="fas fa-comments"></i>
        ...
        <div class="db-status-grid">
            <div class="db-status-item">
                <div class="db-status-left">
                    <span class="db-status-dot {{ ($webServerStatus ?? false) ? 'db-status-dot--green' : 'db-status-dot--red' }}"></span>
                    <span class="db-status-name">Веб сервер</span>
                </div>
                <span class="db-status-badge {{ ($webServerStatus ?? false) ? 'db-status-badge--green' : 'db-status-badge--red' }}">
                    {{ ($webServerStatus ?? false) ? 'Активен' : 'Недостапен' }}
                </span>
            </div>
            <div class="db-status-item">
                <div class="db-status-left">
                    <span class="db-status-dot {{ ($databaseStatus ?? false) ? 'db-status-dot--green' : 'db-status-dot--red' }}"></span>
                    <span class="db-status-name">База на податоци</span>
                </div>
                <span class="db-status-badge {{ ($databaseStatus ?? false) ? 'db-status-badge--green' : 'db-status-badge--red' }}">
                    {{ ($databaseStatus ?? false) ? 'Поврзана' : 'Грешка' }}
                </span>
            </div>
            <div class="db-status-item">
                <div class="db-status-left">
                    <span class="db-status-dot {{ ($storageStatus ?? false) ? 'db-status-dot--green' : 'db-status-dot--red' }}"></span>
                    <span class="db-status-name">Галерија и содржини</span>
                </div>
                <span class="db-status-badge {{ ($storageStatus ?? false) ? 'db-status-badge--green' : 'db-status-badge--red' }}">
                    {{ ($storageStatus ?? false) ? 'Синхронизирани' : 'Грешка' }}
                </span>
            </div>
            <div class="db-status-item">
                <div class="db-status-left">
                    <span class="db-status-dot {{ ($handcraftsStatus ?? false) ? 'db-status-dot--green' : 'db-status-dot--red' }}"></span>
                    <span class="db-status-name">Рачни изработки</span>
                </div>
                <span class="db-status-badge {{ ($handcraftsStatus ?? false) ? 'db-status-badge--green' : 'db-status-badge--red' }}">
                    {{ ($handcraftsStatus ?? false) ? 'Активни' : 'Грешка' }}
                </span>
            </div>
        </div>
    </div>
...
{{-- ═══════════════════════════════════════════════════════════
     ROW 4 — RECENT ACTIVITIES + ANNOUNCEMENTS
═══════════════════════════════════════════════════════════ --}}
<div class="db-grid-2col mb-section">
    ...
    <div class="db-card">
        <div class="db-card-header db-card-header--between">
            ...
            <a href="{{ route('admin.main-activities.index') }}" class="db-card-link">Види сите →</a>
        </div>
        <ul class="db-feed">
            @forelse ($recentActivities ?? [] as $activity)
                <li class="db-feed-item">
                    ...
                    <div class="db-feed-body">
                        <p class="db-feed-title">{{ $activity->title ?? '' }}</p>
                        <p class="db-feed-meta">{{ $activity->created_at?->diffForHumans() ?? 'Нема датум' }}</p>
                    </div>
                </li>
            @empty
    ...
    <div class="db-card">
        <div class="db-card-header db-card-header--between">
            ...
            <a href="{{ route('admin.announcements.index') }}" class="db-card-link">Види сите →</a>
        </div>
        <ul class="db-feed">
            @forelse ($recentAnnouncements ?? [] as $announcement)
                <li class="db-feed-item">
                    ...
                    <div class="db-feed-body">
                        <p class="db-feed-title">{{ $announcement->title ?? '' }}</p>
                        <p class="db-feed-meta">{{ $announcement->published_at?->diffForHumans() ?? $announcement->created_at?->diffForHumans() ?? 'Нема датум' }}</p>
                    </div>
                </li>
            @empty
...
{{-- ═══════════════════════════════════════════════════════════
     ROW 5 — RECENT VISITS + HANDCRAFTS
═══════════════════════════════════════════════════════════ --}}
<div class="db-grid-2col">
    ...
    <div class="db-card">
        <div class="db-card-header db-card-header--between">
            ...
            @if($isAdmin)
                <a href="{{ route('admin.visit-requests') }}" class="db-card-link">Види сите →</a>
            @else
                <a href="{{ route('admin.visit-schedules') }}" class="db-card-link">Отвори распоред →</a>
            @endif
        </div>
        <ul class="db-feed">
            @forelse ($recentVisitRequests ?? [] as $request)
                <li class="db-feed-item db-feed-item--between">
                    <div class="db-feed-body">
                        <p class="db-feed-title">{{ trim(($request->visitor_first_name ?? '') . ' ' . ($request->visitor_last_name ?? '')) }}</p>
                        <p class="db-feed-meta">{{ $request->visit_date?->format('d.m.Y') ?? 'Без датум' }} · {{ $request->visitSchedule?->group_name ?? 'Без распоред' }}</p>
                    </div>
                    <span class="db-status-pill">{{ $request->status ?? '—' }}</span>
                </li>
            @empty
    ...
    <div class="db-card">
        <div class="db-card-header db-card-header--between">
            ...
            <a href="{{ route('admin.izrabotki') }}" class="db-card-link">Уреди страница →</a>
        </div>
        <ul class="db-feed">
            @forelse ($recentHandcrafts ?? [] as $handcraft)
                <li class="db-feed-item">
                    ...
                    <div class="db-feed-body">
                        <p class="db-feed-title">{{ $handcraft->title_mk ?? '' }}</p>
                        <p class="db-feed-meta">{{ $handcraft->published_at?->diffForHumans() ?? $handcraft->created_at?->diffForHumans() ?? 'Нема датум' }}</p>
                    </div>
                </li>
            @empty
...
{{-- ═══════════════════════════════════════════════════════════
     SCOPED STYLES — paste into your CSS file or a <style> block
═══════════════════════════════════════════════════════════ --}}
<style>
/* ── Tokens ─────────────────────────────────────────── */
:root {
