...
<div class="grid grid-cols-1 xl:grid-cols-[300px_1fr] gap-6 items-start">
    ...
    <div class="bg-white border border-gray-100 rounded-2xl overflow-hidden">
        ...
        <form method="POST" action="{{ route('admin.visit-schedules.store') }}" class="p-6 flex flex-col gap-4">
            @csrf
            ...
            <div>
                ...
                <input type="text" name="days_label" value="{{ old('days_label') }}"
                    class="w-full px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-sm text-gray-900
                           focus:outline-none focus:border-gray-400 focus:ring-2 focus:ring-black/5 transition-all"
            ...
            <div>
                ...
                <input type="text" name="time_range" value="{{ old('time_range') }}"
                    class="w-full px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-sm text-gray-900
                           focus:outline-none focus:border-gray-400 focus:ring-2 focus:ring-black/5 transition-all"
        ...
        <div class="mt-4 overflow-x-auto">
            <table class="w-full min-w-[580px]">
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse($visitSchedules as $vs)
                        <tr class="hover:bg-gray-50/60 transition-colors">
                            <td class="px-5 py-3.5">
                                <p class="text-sm font-medium text-gray-900">{{ $vs->group_name }}</p>
                                <p class="text-[11px] text-gray-400 mt-0.5">Редослед #{{ $vs->sort_order }}</p>
                            </td>
                            <td class="px-4 py-3.5 text-sm text-gray-600">{{ $vs->days_label }}</td>
                            ...
                            <td class="px-4 py-3.5">
                                <div class="flex items-center gap-2">
                                        ...
                                        <div x-show="open" x-cloak @keydown.escape.window="open = false"
                                            ...
                                            <div @click.outside="open = false"
                                                ...
                                                <div class="flex items-center justify-between">
                                                    <h4 class="text-sm font-semibold text-gray-900">Уреди: {{ $vs->group_name }}</h4>
                                                    <button @click="open = false" class="text-gray-400 hover:text-gray-600">
                                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>
                                                    </button>
                                                </div>
                                                <form method="POST" action="{{ route('admin.visit-schedules.update', $vs) }}" class="space-y-3">
                                                    @csrf @method('PATCH')
                                                    <input type="text" name="group_name" value="{{ $vs->group_name }}"
                                                    ...
                                                    <input type="text" name="days_label" value="{{ $vs->days_label }}"
                                                        class="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-gray-400" required>
                                                    <input type="text" name="time_range" value="{{ $vs->time_range }}"
                                                        class="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-gray-400" required>
                                                    <input type="number" name="sort_order" value="{{ $vs->sort_order }}"
                                                    ...
                                                    <label class="flex items-center gap-2 text-sm text-gray-600 cursor-pointer">
                                                        <input type="checkbox" name="is_active" value="1" class="accent-gray-900" @checked($vs->is_active)>
                                                        Активна
                                                    </label>
                                                    <button type="submit"
                                    ...
                                    <form method="POST" action="{{ route('admin.visit-schedules.destroy', $vs) }}"
