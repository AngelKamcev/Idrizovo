...
<div class="rounded-xl border border-[#ddd9d0] bg-[#f5f3ef] overflow-hidden">
    ...
    <div class="p-6 space-y-5">
        <div>
            ...
            <input type="text" name="intro_title" value="{{ old('intro_title', $data['intro_title'] ?? '') }}"
        ...
        <div>
            ...
            <textarea name="intro_body" rows="4"
                      class="w-full rounded-lg border border-[#ddd9d0] bg-white px-3 py-2.5 text-sm text-[#0E1B2F] focus:border-[#c9a84c] focus:outline-none focus:ring-1 focus:ring-[#c9a84c]/30" required>{{ old('intro_body', $data['intro_body'] ?? '') }}</textarea>
        </div>
        <div>
            ...
            <textarea name="quote" rows="2"
                      class="w-full rounded-lg border border-[#ddd9d0] bg-white px-3 py-2.5 text-sm text-[#0E1B2F] focus:border-[#c9a84c] focus:outline-none focus:ring-1 focus:ring-[#c9a84c]/30">{{ old('quote', $data['quote'] ?? '') }}</textarea>
        </div>
        <div>
            ...
            <textarea name="scroll_quotes_text" rows="6"
                      class="w-full rounded-lg border border-[#ddd9d0] bg-white px-3 py-2.5 font-mono text-sm text-[#0E1B2F] focus:border-[#c9a84c] focus:outline-none focus:ring-1 focus:ring-[#c9a84c]/30">{{ $scrollText }}</textarea>
        </div>
    </div>
</div>
            ...
            <div class="p-5 space-y-5">
                ...
                <div class="space-y-2.5">
                    <input type="text" name="sections[{{ $i }}][title]"
                           value="{{ old("sections.$i.title", $sec['title'] ?? '') }}"
                           placeholder="Наслов"
                    ...
                    <textarea name="sections[{{ $i }}][body]" rows="4"
                              placeholder="Текст"
                              class="w-full rounded-lg border border-[#ddd9d0] bg-white px-3 py-2.5 text-sm text-[#0E1B2F] focus:border-[#c9a84c] focus:outline-none focus:ring-1 focus:ring-[#c9a84c]/30" required>{{ old("sections.$i.body", $sec['body'] ?? '') }}</textarea>
                </div>
        ...
        <div class="rounded-xl border border-[#ddd9d0] bg-[#f5f3ef] overflow-hidden">
            ...
            <div class="p-5 space-y-3">
                ...
                <input type="file" name="mobile_image_upload_{{ $i }}" accept="image/*"
                       class="block w-full text-xs text-[#666] file:mr-2 file:rounded file:border-0 file:bg-[#0E1B2F] file:px-3 file:py-1.5 file:text-xs file:font-medium file:text-white hover:file:bg-[#1e3251]">
                <input type="text" name="mobile_cards[{{ $i }}][title]"
                       value="{{ old("mobile_cards.$i.title", $card['title'] ?? '') }}"
                       placeholder="Наслов (празно = превод)"
                       class="w-full rounded-lg border border-[#ddd9d0] bg-white px-3 py-2.5 text-sm text-[#0E1B2F] focus:border-[#c9a84c] focus:outline-none focus:ring-1 focus:ring-[#c9a84c]/30">
                <textarea name="mobile_cards[{{ $i }}][body]" rows="3"
                          placeholder="Текст"
                          class="w-full rounded-lg border border-[#ddd9d0] bg-white px-3 py-2.5 text-sm text-[#0E1B2F] focus:border-[#c9a84c] focus:outline-none focus:ring-1 focus:ring-[#c9a84c]/30">{{ old("mobile_cards.$i.body", $card['body'] ?? '') }}</textarea>
            </div>
        </div>
        @endforeach
    </div>
